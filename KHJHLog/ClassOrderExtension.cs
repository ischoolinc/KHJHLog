...
namespace KHJHLog
{
    public static class ClassOrderExtension
    {
        ...
        /// 處理班級順位
        /// </summary>
        /// <param name="ClassOrders"></param>
        private static void FillClassOrder(List<ClassOrder> ClassOrders)
        {
            int order = 1;
            
            //  排序方式：編班人數小至大>沒有特殊生排前>班級名稱
            foreach(ClassOrder co in (from data in ClassOrders orderby data.ClassStudentCountValue ascending,data.hasNumberReduce ascending,data.ClassName ascending select data).ToList())
            {
                co.ClassOrderNumber = order;
                order++;
            }
        }
